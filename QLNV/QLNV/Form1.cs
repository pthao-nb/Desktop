...
namespace QLNV
{
    ...
    {
        String connectstring = "Data Source=LAPTOP-N50OPETB;Initial Catalog=quanlynhanvien;Integrated Security=True;Encrypt=False";
        SqlConnection con;
        ...
        private void btnDangNhap_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlCommand cmd_DangNhap = new SqlCommand(query, con))
                {
                    // Truyền tham số để tránh SQL Injection
                    cmd_DangNhap.Parameters.AddWithValue("@username", username);
                    cmd_DangNhap.Parameters.AddWithValue("@password", password);

                    int result = (int)cmd_DangNhap.ExecuteScalar(); // Trả về số dòng thỏa mãn điều kiện
